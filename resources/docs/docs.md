...
The `home-routes` handler in the `ride-hailing.routes.home` namespace
...
The `home-page` function will in turn call the `ride-hailing.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `ride-hailing.handler` namespace:
...
Request middleware functions are located under the `ride-hailing.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `ride-hailing.dev-middleware` namespace found in
